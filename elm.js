...
Elm.Cell.make = function (_elm) {
   ...
   var toggleFlag = function (cell) {
      return _U.replace([["isFlagged"
                         ,$Basics.not(cell.isFlagged)]],
      cell);
   };
   var open = function (cell) {
      return _U.replace([["isOpen"
                         ,true]],
      cell);
   };
   var addNeighbors = F2(function (cell,
   count) {
      return _U.replace([["neighbors"
                         ,count]],
      cell);
   });
   var makeBomb = function (cell) {
...
Elm.Grid.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var toggleFlag = F2(function (xy,
   grid) {
      return function () {
         var flagCell = F2(function (cellXY,
         cell) {
            return _U.eq(xy,
            cellXY) ? $Cell.toggleFlag(cell) : cell;
         });
   ...
   var countNeighbors = F3(function (cells,
   xy,
   cell) {
      return function () {
         var countIfBomb = F2(function (c,
         count) {
            return $Cell.isBomb(c) ? count + 1 : count;
         });
         var toCell = function (neighborXY) {
            return $Maybe.withDefault($Cell.empty)(A2($Dict.get,
            neighborXY,
            cells));
         };
         var neighborCells = A2($List.map,
         toCell,
         neighbors(xy));
         var bombCount = A3($List.foldl,
         countIfBomb,
         0,
         neighborCells);
         return A2($Cell.addNeighbors,
         cell,
         bombCount);
      }();
   });
   var countAllNeighbors = function (cells) {
      return A2($Dict.map,
   ...
   grid) {
      ...
      F3(function (_v8,cell,count) {
         return function () {
            return test(cell) ? count + 1 : count;
         }();
      }),
      0,
      grid.cells);
   });
   var get = F2(function (xy,
   grid) {
      return A2($Dict.get,
      xy,
      grid.cells);
   });
   var generateBomb = F3(function (grid,
   seed,
   bombs) {
      return function () {
         var randomY = A2($Random.$int,
         0,
         grid.height - 1);
         var randomX = A2($Random.$int,
         0,
         grid.width - 1);
         var randomXY = A2($Random.pair,
         randomX,
         randomY);
         var randomXYAndSeed = A2($Random.generate,
         randomXY,
         seed);
         var bomb = $Cell.makeBomb($Maybe.withDefault($Cell.empty)(A2($Basics.flip,
         get,
         grid)($Basics.fst(randomXYAndSeed))));
         return {ctor: "_Tuple2"
                ,_0: $Basics.snd(randomXYAndSeed)
                ,_1: A3($Dict.insert,
                $Basics.fst(randomXYAndSeed),
                bomb,
                bombs)};
      }();
   });
   var generateBombs = F4(function (grid,
   count,
   seed,
   bombs) {
      return _U.eq($List.length($Dict.keys(bombs)),
      count) ? {ctor: "_Tuple2"
               ,_0: seed
               ,_1: bombs} : A2($Basics.uncurry,
      A2(generateBombs,grid,count),
      A3(generateBomb,
      grid,
      seed,
      bombs));
   });
   var addBombsAndCountNeighbors = F3(function (count,
   seed,
   grid) {
      return function () {
         var seedAndBombs = A4(generateBombs,
         grid,
         count,
         seed,
         $Dict.empty);
         return F2(function (v0,v1) {
            return {ctor: "_Tuple2"
                   ,_0: v0
                   ,_1: v1};
         })($Basics.fst(seedAndBombs))(countAllNeighbors($Dict.union($Basics.snd(seedAndBombs))(grid.cells)));
      }();
   });
   var placeBombs = F3(function (count,
   seed,
   grid) {
      return function () {
         var seedAndCells = A3(addBombsAndCountNeighbors,
         count,
         seed,
         grid);
         return {ctor: "_Tuple2"
                ,_0: $Basics.fst(seedAndCells)
                ,_1: _U.replace([["cells"
                                 ,$Basics.snd(seedAndCells)]],
                grid)};
      }();
   });
   var openCells = F3(function (xys,
   visited,
   grid) {
      return function () {
         var allVisited = A2($Set.union,
         xys,
         visited);
         var inGrid = F2(function (_v10,
         grid) {
            return function () {
               switch (_v10.ctor)
               {case "_Tuple2":
                  return _U.cmp(_v10._0,
                    0) > -1 && (_U.cmp(_v10._0,
                    grid.width) < 0 && (_U.cmp(_v10._1,
                    0) > -1 && _U.cmp(_v10._1,
                    grid.height) < 0));}
               _U.badCase($moduleName,
               "on line 184, column 7 to 60");
            }();
         });
         var neighboringFlags = function (neighborXY) {
            return A2($List.foldl,
            F2(function (cell,count) {
               return $Cell.isFlagged(cell) ? count + 1 : count;
            }),
            0)($List.filterMap(A2($Basics.flip,
            get,
            grid))(neighbors(neighborXY)));
         };
         var cell = function (neighborXY) {
            return A2($Maybe.withDefault,
            $Cell.empty,
            A2(get,neighborXY,grid));
         };
         var toNeighbors = function (neighborXY) {
            return $Cell.neighbors(cell(neighborXY));
         };
         var isBomb = function (neighborXY) {
            return $Cell.isBomb(cell(neighborXY));
         };
         var isReopenable = function (neighborXY) {
            return $Cell.isOpen(cell(neighborXY)) && (_U.cmp(toNeighbors(neighborXY),
            0) > 0 && _U.eq(toNeighbors(neighborXY),
            neighboringFlags(neighborXY)));
         };
         var spread = function (neighborXY) {
            return isBomb(neighborXY) ? _L.fromArray([]) : isReopenable(neighborXY) ? $List.filter(function (xy) {
               return $Basics.not($Cell.isFlagged(A2($Maybe.withDefault,
               $Cell.empty,
               A2(get,xy,grid))));
            })(neighbors(neighborXY)) : _U.cmp(toNeighbors(neighborXY),
            0) > 0 ? _L.fromArray([]) : neighbors(neighborXY);
         };
         var nextXYs = $Set.fromList($List.filter(function (xy) {
            return $Basics.not(A2($Set.member,
            xy,
            allVisited));
         })($List.filter(A2($Basics.flip,
         inGrid,
         grid))($List.concatMap(spread)($Set.toList(xys)))));
         return $Set.isEmpty(nextXYs) ? allVisited : A3(openCells,
         nextXYs,
         allVisited,
         grid);
      }();
   });
   var open = F2(function (xy,
   grid) {
      return function () {
         var xysToOpen = A3(openCells,
         $Set.singleton(xy),
         $Set.empty,
         grid);
         var openCell = F2(function (cellXY,
         cell) {
            return A2($Set.member,
            cellXY,
            xysToOpen) ? $Cell.open(cell) : cell;
         });
         return _U.replace([["cells"
                            ,A2($Dict.map,
                            openCell,
   ...
   var height = function (grid) {
      return grid.height;
   };
   var width = function (grid) {
      return grid.width;
   };
   ...
   var init = F2(function (width,
   height) {
      return function () {
         var generateCells = F2(function (w,
         h) {
            return $Dict.fromList($Array.toList(A2($Array.initialize,
            w * h,
            function (i) {
               return {ctor: "_Tuple2"
                      ,_0: {ctor: "_Tuple2"
                           ,_0: A2($Basics._op["%"],i,w)
                           ,_1: i / w | 0}
                      ,_1: $Cell.empty};
            })));
         });
...
Elm.IncendiaryLocator = Elm.IncendiaryLocator || {};
Elm.IncendiaryLocator.make = function (_elm) {
   "use strict";
   _elm.IncendiaryLocator = _elm.IncendiaryLocator || {};
   if (_elm.IncendiaryLocator.values)
   return _elm.IncendiaryLocator.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "IncendiaryLocator",
   $Basics = Elm.Basics.make(_elm),
   $Cell = Elm.Cell.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   var mousePositions = $Signal.dropRepeats(A2($Signal.sampleOn,
   $Time.fps(30),
   $Mouse.position));
   var window = $Window.dimensions;
   var seconds = $Time.every($Time.second);
   var cellBomb = _L.fromArray([A2($Graphics$Collage.filled,
                               A3($Color.rgb,0,0,0),
                               A2($Graphics$Collage.rect,3,24))
                               ,A2($Graphics$Collage.move,
                               {ctor: "_Tuple2",_0: 0,_1: 12},
                               A2($Graphics$Collage.filled,
                               A3($Color.rgb,0,0,0),
                               $Graphics$Collage.circle(1.5)))
                               ,A2($Graphics$Collage.move,
                               {ctor: "_Tuple2",_0: 0,_1: -12},
                               A2($Graphics$Collage.filled,
                               A3($Color.rgb,0,0,0),
                               ...
                               $Graphics$Collage.circle(2)))]);
   var cellContents = function (cell) {
      return function () {
         var neighbors = $Cell.neighbors(cell);
         var color = function () {
            switch (neighbors)
            {case 1: return A3($Color.rgb,
               ...
               case 5: return A3($Color.rgb,
                 ...
                 0,
                 0);
               case 6: return A3($Color.rgb,
                 0,
                 127,
                 127);
               case 7: return A3($Color.rgb,
                 0,
                 0,
                 0);}
            return A3($Color.rgb,
         ...
         var toText = function (n) {
            return $Graphics$Collage.moveY(3)($Graphics$Collage.text($Text.color(color)($Text.height(22)($Text.fromString($Basics.toString(n))))));
         };
         return $Cell.isBomb(cell) ? cellBomb : _U.eq(neighbors,
         0) ? _L.fromArray([]) : _L.fromArray([toText(neighbors)]);
      }();
   };
   var gameButtonFace = function (model) {
      return function () {
         var surprised = $Graphics$Collage.group(_L.fromArray([A2($Graphics$Collage.move,
                                                              {ctor: "_Tuple2"
                                                              ,_0: -8
                                                              ,_1: 4},
                                                              A2($Graphics$Collage.filled,
                                                              ...
                                                              6,
                                                              8)))]));
         var smile = $Graphics$Collage.toForm(A3($Graphics$Collage.collage,
         16,
         6,
         ...
         _U.replace([["width",2]],
         $Graphics$Collage.defaultLine),
         $Graphics$Collage.circle(8)))])));
         var happy = $Graphics$Collage.group(_L.fromArray([A2($Graphics$Collage.move,
                                                          {ctor: "_Tuple2"
                                                          ,_0: -8
                                                          ,_1: 4},
                                                          A2($Graphics$Collage.filled,
                                                          ...
                                                          -8,
                                                          smile)]));
         var cool = $Graphics$Collage.group(_L.fromArray([A2($Graphics$Collage.move,
                                                         {ctor: "_Tuple2"
                                                         ,_0: -8
                                                         ,_1: 4},
                                                         A2($Graphics$Collage.filled,
                                                         A3($Color.rgb,0,0,0),
                                                         $Graphics$Collage.circle(6)))
                                                         ,A2($Graphics$Collage.move,
                                                         {ctor: "_Tuple2"
                                                         ,_0: 8
                                                         ,_1: 4},
                                                         ...
                                                         -8,
                                                         smile)]));
         var sad = $Graphics$Collage.group(_L.fromArray([A2($Graphics$Collage.traced,
                                                        _U.replace([["width"
                                                                    ,2]],
                                                        $Graphics$Collage.defaultLine),
         ...
         var expression = function () {
            ...
            switch (_v1.ctor)
            {case "Over":
               return _v1._0 ? cool : sad;}
            return function () {
               var _v3 = model.clicking;
               switch (_v3.ctor)
               {case "Just": return surprised;
                  case "Nothing": return happy;}
               _U.badCase($moduleName,
               "between lines 467 and 472");
            }();
         }();
         return $Graphics$Collage.group(_L.fromArray([A2($Graphics$Collage.filled,
                                                     A3($Color.rgb,255,255,0),
   ...
   var cellSize = 32;
   var cellBackground = F2(function (highlighted,
   cell) {
      return function () {
         var color = $Cell.isBomb(cell) && $Cell.isOpen(cell) ? A3($Color.rgb,
         255,
         0,
         0) : highlighted && _U.cmp($Cell.neighbors(cell),
         1) > -1 ? A3($Color.rgb,
         255,
         255,
         0) : A3($Color.rgb,189,189,189);
         return _L.fromArray([A2($Graphics$Collage.filled,
         color,
         $Graphics$Collage.square($Basics.toFloat(cellSize - 2)))]);
      }();
   });
   var cellCover = F3(function (highlighted,
   clicked,
   cell) {
      return function () {
         var flag = $Cell.isFlagged(cell) ? _L.fromArray([A2($Graphics$Collage.moveY,
                                                         -10,
                                                         $Graphics$Collage.toForm(A3($Graphics$Collage.collage,
                                                         20,
                                                         4,
                                                         _L.fromArray([A2($Graphics$Collage.moveY,
                                                         -2,
                                                         ...
                                                         $Graphics$Collage.polygon(_L.fromArray([{ctor: "_Tuple2"
                                                                                                ...
                                                                                                ,{ctor: "_Tuple2"
                                                                                                 ,_0: -10
                                                                                                 ,_1: 6}])))]) : _L.fromArray([]);
         var shadow = highlighted ? A3($Color.rgb,
         170,
         170,
         0) : A3($Color.rgb,122,122,122);
         var color = highlighted ? A3($Color.rgb,
         255,
         255,
         0) : A3($Color.rgb,189,189,189);
         var background = A2($Graphics$Collage.filled,
         color,
         $Graphics$Collage.square($Basics.toFloat(cellSize)));
         var cover = $Graphics$Collage.group(A2($List._op["::"],
         background,
         flag));
         return $Cell.isOpen(cell) ? _L.fromArray([]) : clicked ? _L.fromArray([cover]) : _L.fromArray([cover
                                                                                                       ,A2($Graphics$Collage.filled,
                                                                                                       A3($Color.rgb,
                                                                                                       255,
                                                                                                       255,
   ...
   var gridSize = 258;
   var gridBackground = $Graphics$Collage.filled(A3($Color.rgb,
   122,
   122,
   122))($Graphics$Collage.square($Basics.toFloat(gridSize)));
   var gridCell = F2(function (_v5,
   model) {
      return function () {
         switch (_v5.ctor)
         {case "_Tuple2":
            return function () {
                 var cell = A2($Maybe.withDefault,
                 $Cell.empty,
                 A2($Grid.get,
                 {ctor: "_Tuple2"
                 ,_0: _v5._0
                 ,_1: _v5._1},
                 model.grid));
                 var click = A2($Maybe.withDefault,
                 {ctor: "_Tuple2"
                 ,_0: model.grid.width + 1
                 ,_1: model.grid.height + 1},
                 model.clicking);
                 var clicked = _U.eq($Basics.fst(click),
                 _v5._0) && _U.eq($Basics.snd(click),
                 _v5._1);
                 var highlight = A2($Maybe.withDefault,
                 {ctor: "_Tuple2"
                 ,_0: model.grid.width + 1
                 ,_1: model.grid.height + 1},
                 model.highlight);
                 var highlighted = _U.eq($Basics.fst(highlight),
                 _v5._0) && _U.eq($Basics.snd(highlight),
                 _v5._1);
                 var yOffset = $Basics.toFloat($Basics.negate((gridSize / 2 | 0) * -1 + _v5._1 * cellSize + (cellSize / 2 | 0) + 1));
                 var xOffset = $Basics.toFloat((gridSize / 2 | 0) * -1 + _v5._0 * cellSize + (cellSize / 2 | 0) + 1);
                 return $Graphics$Collage.move({ctor: "_Tuple2"
                                               ,_0: xOffset
                                               ,_1: yOffset})($Graphics$Collage.group($List.concat(_L.fromArray([A2(cellBackground,
                                                                                                                highlighted,
                                                                                                                cell)
                                                                                                                ,cellContents(cell)
                                                                                                                ,A3(cellCover,
         ...
         _U.badCase($moduleName,
         "between lines 504 and 530");
      }();
   });
   var gridCells = function (model) {
      ...
      function (y) {
         ...
         _L.range(1,model.grid.width));
      },
      _L.range(1,model.grid.height));
   };
   var gameButtonSize = 64;
   var gameButton = function (model) {
      return function () {
         var size = $Basics.toFloat(gameButtonSize);
         var clicked = function () {
            var _v9 = model.clicking;
            switch (_v9.ctor)
            {case "Just":
               return _U.eq(_v9._0,
                 {ctor: "_Tuple2"
               ...
               case "Nothing": return false;}
            _U.badCase($moduleName,
            "between lines 382 and 387");
         }();
         var highlighted = function () {
            var _v11 = model.highlight;
            switch (_v11.ctor)
            {case "Just":
         ...
         255,
         0) : A3($Color.rgb,189,189,189);
         var cover = A2($Graphics$Collage.filled,
         color,
         ...
         170,
         0) : A3($Color.rgb,122,122,122);
         return clicked ? $Graphics$Collage.group(_L.fromArray([cover
                                                               ,gameButtonFace(model)])) : $Graphics$Collage.group(_L.fromArray([cover
                                                                                                                                ,A2($Graphics$Collage.filled,
                                                                                                                                A3($Color.rgb,
                                                                                                                                255,
                                                                                                                                255,
   ...
   var dashboardHeight = gameButtonSize;
   var dashboardWidth = gridSize;
   var boardHeight = gridSize + dashboardHeight + 20 * 3;
   var dashboard = function (model) {
      return $Graphics$Collage.moveY($Basics.toFloat((boardHeight / 2 | 0) - ((gameButtonSize / 2 | 0) + 20)))($Graphics$Collage.group(_L.fromArray([gameButton(model)])));
   };
   var grid = function (model) {
      return $Graphics$Collage.moveY($Basics.toFloat((boardHeight / 2 | 0) * -1 + ((gridSize / 2 | 0) + 20)))($Graphics$Collage.group(A2($List._op["::"],
      gridBackground,
      gridCells(model))));
   };
   var boardWidth = gridSize + 20 * 2;
   var chrome = function () {
      var grid = $Basics.toFloat(gridSize);
      var dashHeight = $Basics.toFloat(dashboardHeight);
      var height = $Basics.toFloat(boardHeight);
      var width = $Basics.toFloat(boardWidth);
      return $Graphics$Collage.group(_L.fromArray([A2($Graphics$Collage.filled,
                                                  A3($Color.rgb,189,189,189),
                                                  A2($Graphics$Collage.rect,
                                                  ...
                                                  $Graphics$Collage.polygon(_L.fromArray([{ctor: "_Tuple2"
                                                                                          ,_0: width / 2 * -1 + 17
                                                                                          ,_1: height / 2 - (20 + dashHeight + 3)}
                                                                                         ,{ctor: "_Tuple2"
                                                                                          ,_0: width / 2 * -1 + 20
                                                                                          ,_1: height / 2 - (20 + dashHeight)}
                                                                                         ,{ctor: "_Tuple2"
                                                                                          ,_0: width / 2 * -1 + 20
                                                                                          ,_1: height / 2 - 20}
                                                                                         ,{ctor: "_Tuple2"
                                                  ...
                                                  $Graphics$Collage.polygon(_L.fromArray([{ctor: "_Tuple2"
                                                                                         ...
                                                                                         ,{ctor: "_Tuple2"
                                                                                          ,_0: width / 2 * -1 + 20
                                                                                          ,_1: height / 2 * -1 + 20 + grid}
                                                                                         ,{ctor: "_Tuple2"
                                                                                          ,_0: width / 2 - 20
                                                                                          ,_1: height / 2 * -1 + 20 + grid}
                                                                                         ,{ctor: "_Tuple2"
                                                                                          ,_0: width / 2 - 17
                                                                                          ,_1: height / 2 * -1 + 20 + grid + 3}
                                                                                         ,{ctor: "_Tuple2"
                                                                                          ,_0: width / 2 * -1 + 17
                                                                                          ,_1: height / 2 * -1 + 20 + grid + 3}])))
                                                  ,A2($Graphics$Collage.filled,
   ...
   model) {
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 var content = function () {
                    var _v17 = model.status;
                    switch (_v17.ctor)
                    {case "Loading":
                       return $Graphics$Element.centered($Text.color(A3($Color.rgb,
                         255,
                         255,
                         255))($Text.height(40)($Text.fromString("Loading..."))));}
                    return board(model);
                 }();
                 var instructions = $Graphics$Element.leftAligned($Text.color(A3($Color.rgb,
                 220,
                 220,
                 220))($Text.height(20)(A2($Basics.flip,
                 $Text.append,
                 $Text.fromString("* Click the face to restart"))(A2($Basics.flip,
                 $Text.append,
                 $Text.fromString("* Press space over a cell to flag it\n"))($Text.fromString("* Click cells to open\n"))))));
                 var title = $Graphics$Element.centered($Text.color(A3($Color.rgb,
                 255,
                 255,
                 255))($Text.height(40)($Text.fromString("Incendiary Locator"))));
         ...
         _U.badCase($moduleName,
         "between lines 211 and 240");
      }();
   });
   var updateFlagging = F2(function (space,
   model) {
      return function () {
         var _v18 = model.highlight;
         switch (_v18.ctor)
         {case "Just":
            return space ? _U.replace([["grid"
                                       ,A2($Grid.toggleFlag,
                                       _v18._0,
                                       model.grid)]],
              model) : model;
   ...
   var updateHighlight = F3(function (_v20,
   _v21,
   model) {
      return function () {
         switch (_v21.ctor)
         {case "_Tuple2":
            return function () {
                 ...
                 {case "_Tuple2":
                    return function () {
                         var gameYStart = (_v20._1 / 2 | 0) - (gameButtonSize / 2 | 0) - ((boardHeight / 2 | 0) - ((gameButtonSize / 2 | 0) + 20));
                         var gameXStart = (_v20._0 / 2 | 0) - (gameButtonSize / 2 | 0);
                         var onGameButton = _U.cmp(_v21._0,
                         gameXStart) > -1 && (_U.cmp(_v21._0,
                         gameXStart + gameButtonSize) < 0 && (_U.cmp(_v21._1,
                         gameYStart) > -1 && _U.cmp(_v21._1,
                         gameYStart + gameButtonSize) < 0));
                         var borderlessGridSize = gridSize - 2;
                         var gridXStart = (_v20._0 / 2 | 0) - (borderlessGridSize / 2 | 0);
                         var cellX = (_v21._0 - gridXStart) / cellSize | 0;
                         var gridYStart = (_v20._1 / 2 | 0) - (borderlessGridSize / 2 | 0) - ((boardHeight / 2 | 0) * -1 + ((gridSize / 2 | 0) + 20));
                         var cellY = (_v21._1 - gridYStart) / cellSize | 0;
                         var inGrid = _U.cmp(_v21._0,
                         gridXStart) > -1 && (_U.cmp(_v21._0,
                         gridXStart + borderlessGridSize) < 0 && (_U.cmp(_v21._1,
                         gridYStart) > -1 && _U.cmp(_v21._1,
                         gridYStart + borderlessGridSize) < 0));
                         return inGrid ? _U.replace([["highlight"
                                                     ,$Maybe.Just({ctor: "_Tuple2"
                                                                  ,_0: cellX
                                                                  ,_1: cellY})]],
                         model) : onGameButton ? _U.replace([["highlight"
                                                             ,$Maybe.Just({ctor: "_Tuple2"
                                                                          ,_0: -1
                                                                          ,_1: -1})]],
                         model) : _U.replace([["highlight"
                                              ,$Maybe.Nothing]],
                         model);
                      }();}
                 _U.badCase($moduleName,
                 "between lines 86 and 115");
              }();}
         _U.badCase($moduleName,
         "between lines 86 and 115");
      }();
   });
   var updateSeed = F2(function (time,
   model) {
      return function () {
         var seedAndGrid = A3($Grid.placeBombs,
         10,
         ...
         return function () {
            var _v28 = model.seed;
            switch (_v28.ctor)
            {case "Just": return model;
               case "Nothing":
               return _U.replace([["grid"
                                  ,$Basics.snd(seedAndGrid)]
                                 ,["seed"
                                  ,$Maybe.Just($Basics.fst(seedAndGrid))]],
                 model);}
            _U.badCase($moduleName,
   ...
   var Playing = function (a) {
      return {ctor: "Playing"
             ,_0: a};
   };
   var Ready = {ctor: "Ready"};
   var updateClicking = F2(function (down,
   model) {
      return function () {
         var _v30 = model.clicking;
         switch (_v30.ctor)
         {case "Just":
            return $Basics.not(down) ? _U.eq(model.clicking,
              model.highlight) ? _U.eq(_v30._0,
              {ctor: "_Tuple2"
              ,_0: -1
              ,_1: -1}) ? function () {
                 var nextSeed = A2($Maybe.withDefault,
                 $Random.initialSeed(0),
                 model.seed);
                 var seedAndGrid = A2($Grid.placeBombs,
                 10,
                 nextSeed)(A2($Grid.init,
                 $Grid.width(model.grid),
                 $Grid.height(model.grid)));
                 return _U.replace([["clicking"
                                    ,$Maybe.Nothing]
                                   ,["grid"
                                    ,$Basics.snd(seedAndGrid)]
                                   ,["seed"
              ...
              model) : model;
            case "Nothing":
            return down ? _U.replace([["clicking"
                                      ,model.highlight]],
              model) : model;}
         _U.badCase($moduleName,
         "between lines 120 and 155");
      }();
   });
   var updateStatus = F2(function (time,
   model) {
      return function () {
         ...
         {case "Loading":
            ...
            case "Over": return model;
            case "Playing":
            return function () {
                 var won = $List.all(function (cell) {
                    return $Cell.isOpen(cell) || $Cell.isBomb(cell) && $Cell.isFlagged(cell);
                 })($Dict.values($Grid.cells(model.grid)));
                 var lost = $List.any(function (cell) {
                    return $Cell.isOpen(cell) && $Cell.isBomb(cell);
                 })($Dict.values($Grid.cells(model.grid)));
                 return lost ? _U.replace([["status"
                                           ,Over(false)]],
                 model) : won ? _U.replace([["status"
                                            ,Over(true)]],
                 model) : model;
              }();
            case "Ready":
            return _U.cmp(A2($Grid.count,
              $Cell.isOpen,
              model.grid),
              0) > 0 ? _U.replace([["status"
                                   ,Playing(time)]],
              model) : model;}
         _U.badCase($moduleName,
         "between lines 172 and 204");
      }();
   });
   var update = F2(function (_v39,
   model) {
      return function () {
         return function () {
            var _v41 = model.status;
            switch (_v41.ctor)
            {case "Loading":
               return updateStatus(_v39.time)(A2(updateSeed,
                 _v39.time,
                 model));
               case "Over":
               return updateClicking(_v39.button)(A3(updateHighlight,
                 _v39.window,
                 _v39.xy,
                 model));}
            return updateStatus(_v39.time)(updateFlagging(_v39.space)(updateClicking(_v39.button)(A3(updateHighlight,
            _v39.window,
            _v39.xy,
            model))));
         }();
   ...
   window,
   state);
   _elm.IncendiaryLocator.values = {_op: _op
                                   ,Loading: Loading
                                   ,Ready: Ready
                                   ,Playing: Playing
                                   ,Over: Over
                                   ...
                                   ,updateClicking: updateClicking
                                   ,updateFlagging: updateFlagging
